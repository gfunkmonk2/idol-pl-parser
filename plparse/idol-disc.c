 ...
 *
 * The rest of the idol-pl-parser API exclusively uses URIs.
 **/
...
static char *
idol_resolve_symlink (const char *device, GError **error)
{
  char *dir, *link;
  char *f;
  char *f1;
			      ...
			      char **mountpoint, GVolume **volume)
{
  ...
  for (l = list; l != NULL; l = l->next) {
    ...
    if (ddev == NULL)
      continue;
    resolved = idol_resolve_symlink (ddev, NULL);
    g_free (ddev);
    if (resolved == NULL)
      continue;
	      ...
	      GError     **error)
{
  ...
  if (g_file_test (local, G_FILE_TEST_IS_DIR) != FALSE) {
	...
	if (data.error) {
	  ...
	  g_set_error (error, IDOL_PL_PARSER_ERROR, IDOL_PL_PARSER_ERROR_MOUNT_FAILED,
  ...
  device = idol_resolve_symlink (local, error);
  g_free (local);
  if (!device)
    return NULL;
  ...
  if (!found) {
    g_set_error (error, IDOL_PL_PARSER_ERROR, IDOL_PL_PARSER_ERROR_NO_DISC,
 ...
 **/
char *
idol_cd_mrl_from_type (const char *scheme, const char *dir)
{
  char *retval;
...
static char *
idol_cd_dir_get_parent (const char *dir)
{
 ...
 * MRL as from idol_cd_mrl_from_type().
 *
 * Note that this function does synchronous I/O.
 *
 * If no disc is present in the drive, a #IDOL_PL_PARSER_ERROR_NO_DISC
 * error will be returned. On unknown mounting errors, a
 * #IDOL_PL_PARSER_ERROR_MOUNT_FAILED error will be returned. On other
 * I/O errors, or if resolution of symlinked mount paths failed, a code from
 * #GIOErrorEnum will be returned.
 *
 * Return value: #IdolDiscMediaType corresponding to the disc's type, or #MEDIA_TYPE_ERROR on failure
 **/
IdolDiscMediaType
idol_cd_detect_type_from_dir (const char *dir, char **mrl, GError **error)
{
  CdCache *cache;
  ...
  if ((type = cd_cache_disc_is_vcd (cache, error)) == MEDIA_TYPE_DATA &&
    ...
    cd_cache_free (cache);
    parent = idol_cd_dir_get_parent (dir);
    if (!parent)
      return type;
  ...
  if (type == MEDIA_TYPE_DVD) {
    *mrl = idol_cd_mrl_from_type ("dvd", cache->mountpoint);
  } else if (type == MEDIA_TYPE_VCD) {
    *mrl = idol_cd_mrl_from_type ("vcd", cache->mountpoint);
  }
 ...
 * Note that this function does synchronous I/O.
 *
 * Possible error codes are as per idol_cd_detect_type_from_dir().
 *
 * Return value: #IdolDiscMediaType corresponding to the disc's type, or #MEDIA_TYPE_ERROR on failure
 **/
IdolDiscMediaType
idol_cd_detect_type_with_url (const char *device,
    			       char      **mrl,
			       GError     **error)
{
  CdCache *cache;
  ...
  case MEDIA_TYPE_DVD:
	...
	str = cache->device;
      *mrl = idol_cd_mrl_from_type ("dvd", str);
    }
    break;
  case MEDIA_TYPE_VCD:
    {
	...
	str = cache->device;
      *mrl = idol_cd_mrl_from_type ("vcd", str);
    }
    break;
  case MEDIA_TYPE_CDDA:
	...
	*mrl = idol_cd_mrl_from_type ("cdda", dev + 5);
      else
	*mrl = idol_cd_mrl_from_type ("cdda", dev);
    }
    break;
 ...
 *
 * Possible error codes are as per idol_cd_detect_type_with_url().
 *
 * Return value: #IdolDiscMediaType corresponding to the disc's type, or #MEDIA_TYPE_ERROR on failure
 **/
IdolDiscMediaType
idol_cd_detect_type (const char  *device,
		      GError     **error)
{
  return idol_cd_detect_type_with_url (device, NULL, error);
}
 ...
 **/
gboolean
idol_cd_has_medium (const char *device)
{
  CdCache *cache;
 ...
 **/
const char *
idol_cd_get_human_readable_name (IdolDiscMediaType type)
{
  switch (type)
  {
  case MEDIA_TYPE_CDDA:
...
GQuark
idol_disc_media_type_quark (void)
{
  static GQuark quark = 0;
  if (!quark)
    quark = g_quark_from_static_string ("idol_disc_media_type");
