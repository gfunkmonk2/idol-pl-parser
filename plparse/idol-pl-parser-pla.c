...
#ifndef IDOL_PL_PARSER_MINI
gboolean
idol_pl_parser_save_pla (IdolPlParser    *parser,
                          ...
                          GError          **error)
{
	...
	if (stream == NULL)
        ...
        num_entries_total = idol_pl_playlist_size (playlist);
	...
	*((gint32 *)buffer) = GINT32_TO_BE (num_entries_total);
	...
	if (idol_pl_parser_write_buffer (G_OUTPUT_STREAM (stream), buffer, RECORD_SIZE, error) == FALSE)
	{
...
IdolPlParserResult
idol_pl_parser_add_pla (IdolPlParser *parser,
			 GFile *file,
			 GFile *base_file,
			 ...
			 gpointer data)
{
	...
	while (offset + RECORD_SIZE <= size && entry < max_entries) {
		...
		idol_pl_parser_add_uri (parser, IDOL_PL_PARSER_FIELD_URI, uri, NULL);
	...
	if (title != NULL)
		idol_pl_parser_playlist_end (parser, title);
