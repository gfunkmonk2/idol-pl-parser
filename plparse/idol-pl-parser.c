...
/**
 * SECTION:idol-pl-parser
 * @short_description: playlist parser
 * @stability: Stable
 * @include: idol-pl-parser.h
 *
 * #IdolPlParser is a general-purpose playlist parser and writer, with
 * support for several different types of playlist. Note that idol-pl-parser requires a main loop
 * to operate properly (e.g. for the #IdolPlParser::entry-parsed signal to be emitted).
 *
 * <example>
 *  <title>Reading a Playlist</title>
 *  <programlisting>
 * IdolPlParser *pl = idol_pl_parser_new ();
 ...
 *
 * if (idol_pl_parser_parse (pl, "http://example.com/playlist.pls", FALSE) != IDOL_PL_PARSER_RESULT_SUCCESS)
 ...
 * g_signal_connect (G_OBJECT (pl), "entry-parsed", G_CALLBACK (entry_parsed), NULL);
 *
 * idol_pl_parser_parse_async (pl, "http://example.com/playlist.pls", FALSE, NULL, parse_cb, NULL);
 * g_object_unref (pl);
 *
 * static void
 * parse_cb (IdolPlParser *parser, GAsyncResult *result, gpointer user_data)
 * {
 *	GError *error = NULL;
 * 	if (idol_pl_parser_parse_finish (parser, result, &error) != IDOL_PL_PARSER_RESULT_SUCCESS) {
 ...
 *  <programlisting>
 * static void
 * entry_parsed (IdolPlParser *parser, const gchar *uri, GHashTable *metadata, gpointer user_data)
 * {
 * 	gchar *title = g_hash_table_lookup (metadata, IDOL_PL_PARSER_FIELD_TITLE);
 * 	if (title != NULL)
 ...
 * 	GFile *file;
 *
 * 	pl = idol_pl_parser_new ();
 * 	playlist = idol_pl_playlist_new ();
 * 	file = g_file_new_for_path ("/tmp/playlist.pls");
 *
 * 	idol_pl_playlist_append (playlist, &iter);
 * 	idol_pl_playlist_set (playlist, &iter,
 * 			       IDOL_PL_PARSER_FIELD_URI, "file:///1.ogg",
 * 			       IDOL_PL_PARSER_FIELD_TITLE, "1.ogg",
 * 			       NULL);
 *
 * 	idol_pl_playlist_append (playlist, &iter);
 * 	idol_pl_playlist_set (playlist, &iter,
 * 			       IDOL_PL_PARSER_FIELD_URI, "file:///2.ogg",
 * 			       NULL);
 *
 * 	if (idol_pl_parser_save (pl, playlist, file, "Title",
 * 				  IDOL_PL_PARSER_PLS, NULL) != TRUE) {
...
#include "idol-pl-parser-wm.h"
#include "idol-pl-parser-qt.h"
#include "idol-pl-parser-pls.h"
#include "idol-pl-parser-xspf.h"
#include "idol-pl-parser-media.h"
#include "idol-pl-parser-smil.h"
#include "idol-pl-parser-pla.h"
#include "idol-pl-parser-podcast.h"
#include "idol-pl-parser-lines.h"
...
#ifndef IDOL_PL_PARSER_MINI
typedef IdolPlParserResult (*PlaylistCallback) (IdolPlParser *parser, GFile *uri, GFile *base_file, IdolPlParseData *parse_data, gpointer data);
#endif
...
static PlaylistTypes special_types[] = {
	PLAYLIST_TYPE ("audio/x-mpegurl", idol_pl_parser_add_m3u, NULL, FALSE),
	PLAYLIST_TYPE ("audio/playlist", idol_pl_parser_add_m3u, NULL, FALSE),
	PLAYLIST_TYPE ("audio/x-scpls", idol_pl_parser_add_pls, NULL, FALSE),
	PLAYLIST_TYPE ("application/x-smil", idol_pl_parser_add_smil, NULL, FALSE),
	PLAYLIST_TYPE ("application/smil", idol_pl_parser_add_smil, NULL, FALSE),
	PLAYLIST_TYPE ("application/vnd.ms-wpl", idol_pl_parser_add_smil, NULL, FALSE),
	PLAYLIST_TYPE ("video/x-ms-wvx", idol_pl_parser_add_asx, NULL, FALSE),
	PLAYLIST_TYPE ("audio/x-ms-wax", idol_pl_parser_add_asx, NULL, FALSE),
	PLAYLIST_TYPE ("application/xspf+xml", idol_pl_parser_add_xspf, NULL, FALSE),
	PLAYLIST_TYPE ("text/uri-list", idol_pl_parser_add_ra, idol_pl_parser_is_uri_list, FALSE),
	PLAYLIST_TYPE ("text/x-google-video-pointer", idol_pl_parser_add_gvp, NULL, FALSE),
	PLAYLIST_TYPE ("text/google-video-pointer", idol_pl_parser_add_gvp, NULL, FALSE),
	PLAYLIST_TYPE ("audio/x-iriver-pla", idol_pl_parser_add_pla, NULL, FALSE),
	PLAYLIST_TYPE ("application/atom+xml", idol_pl_parser_add_atom, NULL, FALSE),
	PLAYLIST_TYPE ("application/rss+xml", idol_pl_parser_add_rss, idol_pl_parser_is_rss, FALSE),
	PLAYLIST_TYPE ("text/x-opml+xml", idol_pl_parser_add_opml, NULL, FALSE),
#ifndef IDOL_PL_PARSER_MINI
	PLAYLIST_TYPE ("application/x-desktop", idol_pl_parser_add_desktop, NULL, TRUE),
	PLAYLIST_TYPE ("application/x-mate-app-info", idol_pl_parser_add_desktop, NULL, TRUE),
	PLAYLIST_TYPE ("application/x-cd-image", idol_pl_parser_add_iso, NULL, TRUE),
	PLAYLIST_TYPE ("application/x-extension-img", idol_pl_parser_add_iso, NULL, TRUE),
	PLAYLIST_TYPE ("application/x-cue", idol_pl_parser_add_cue, NULL, TRUE),
	PLAYLIST_TYPE (DIR_MIME_TYPE, idol_pl_parser_add_directory, NULL, TRUE),
	PLAYLIST_TYPE (BLOCK_DEVICE_TYPE, idol_pl_parser_add_block, NULL, TRUE),
#endif
};
...
static PlaylistTypes dual_types[] = {
	PLAYLIST_TYPE2 ("audio/x-real-audio", idol_pl_parser_add_ra, idol_pl_parser_is_uri_list),
	PLAYLIST_TYPE2 ("audio/x-pn-realaudio", idol_pl_parser_add_ra, idol_pl_parser_is_uri_list),
	PLAYLIST_TYPE2 ("application/ram", idol_pl_parser_add_ra, idol_pl_parser_is_uri_list),
	PLAYLIST_TYPE2 ("application/vnd.rn-realmedia", idol_pl_parser_add_ra, idol_pl_parser_is_uri_list),
	PLAYLIST_TYPE2 ("audio/x-pn-realaudio-plugin", idol_pl_parser_add_ra, idol_pl_parser_is_uri_list),
	PLAYLIST_TYPE2 ("audio/vnd.rn-realaudio", idol_pl_parser_add_ra, idol_pl_parser_is_uri_list),
	PLAYLIST_TYPE2 ("audio/x-realaudio", idol_pl_parser_add_ra, idol_pl_parser_is_uri_list),
	PLAYLIST_TYPE2 ("text/plain", idol_pl_parser_add_ra, idol_pl_parser_is_uri_list),
	PLAYLIST_TYPE2 ("application/x-php", NULL, NULL),
	PLAYLIST_TYPE2 ("audio/x-ms-asx", idol_pl_parser_add_asx, idol_pl_parser_is_asx),
	PLAYLIST_TYPE2 ("video/x-ms-asf", idol_pl_parser_add_asf, idol_pl_parser_is_asf),
	PLAYLIST_TYPE2 ("video/x-ms-wmv", idol_pl_parser_add_asf, idol_pl_parser_is_asf),
	PLAYLIST_TYPE2 ("audio/x-ms-wma", idol_pl_parser_add_asf, idol_pl_parser_is_asf),
	PLAYLIST_TYPE2 ("video/quicktime", idol_pl_parser_add_quicktime, idol_pl_parser_is_quicktime),
	PLAYLIST_TYPE2 ("video/mp4", idol_pl_parser_add_quicktime, idol_pl_parser_is_quicktime),
	PLAYLIST_TYPE2 ("application/x-quicktime-media-link", idol_pl_parser_add_quicktime, idol_pl_parser_is_quicktime),
	PLAYLIST_TYPE2 ("application/x-quicktimeplayer", idol_pl_parser_add_quicktime, idol_pl_parser_is_quicktime),
	PLAYLIST_TYPE2 ("application/xml", idol_pl_parser_add_xml_feed, idol_pl_parser_is_xml_feed),
};

static char *idol_pl_parser_mime_type_from_data (gconstpointer data, int len);

#ifndef IDOL_PL_PARSER_MINI

static void idol_pl_parser_set_property (GObject *object,
					  guint prop_id,
					  const GValue *value,
					  GParamSpec *pspec);
static void idol_pl_parser_get_property (GObject *object,
					  guint prop_id,
					  GValue *value,
					  GParamSpec *pspec);

struct IdolPlParserPrivate {
...
static int idol_pl_parser_table_signals[LAST_SIGNAL];
static GParamSpecPool *idol_pl_parser_pspec_pool = NULL;

static void idol_pl_parser_class_init (IdolPlParserClass *klass);
static void idol_pl_parser_base_class_finalize	(IdolPlParserClass *klass);
static void idol_pl_parser_init       (IdolPlParser *parser);
static void idol_pl_parser_finalize   (GObject *object);

static void idol_pl_parser_init       (IdolPlParser      *self);
static void idol_pl_parser_class_init (IdolPlParserClass *klass);
static gpointer idol_pl_parser_parent_class = NULL;

#define IDOL_PL_PARSER_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), IDOL_TYPE_PL_PARSER, IdolPlParserPrivate))

GType
idol_pl_parser_get_type (void)
{
	static volatile gsize g_define_type_id__volatile = 0;
	if (g_once_init_enter (&g_define_type_id__volatile))
	{
		const GTypeInfo g_define_type_info = {
			sizeof (IdolPlParserClass),
			NULL,
			(GBaseFinalizeFunc) idol_pl_parser_base_class_finalize,
			(GClassInitFunc) idol_pl_parser_class_init,
			NULL,
			NULL,
			sizeof (IdolPlParser),
			0,
			(GInstanceInitFunc) idol_pl_parser_init,
		};
		GType g_define_type_id = g_type_register_static (G_TYPE_OBJECT, "IdolPlParser", &g_define_type_info, 0);
...
static void
idol_pl_parser_class_init (IdolPlParserClass *klass)
{
	GParamSpec *pspec;
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	idol_pl_parser_parent_class = g_type_class_peek_parent (klass);
	if (!g_thread_supported ())
		g_thread_init (NULL);

	g_type_class_add_private (klass, sizeof (IdolPlParserPrivate));

	object_class->finalize = idol_pl_parser_finalize;
	object_class->set_property = idol_pl_parser_set_property;
	object_class->get_property = idol_pl_parser_get_property;

	/* Properties */

	/**
	 * IdolPlParser:recurse:
	 *
	 ...
	 */
	idol_pl_parser_table_signals[ENTRY_PARSED] =
		g_signal_new ("entry-parsed",
			      G_TYPE_FROM_CLASS (klass),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (IdolPlParserClass, entry_parsed),
			      NULL, NULL,
			      idolplparser_marshal_VOID__STRING_BOXED,
			      G_TYPE_NONE, 2, G_TYPE_STRING, IDOL_TYPE_PL_PARSER_METADATA);
	/**
	 * IdolPlParser::playlist-started:
	 * @parser: the object which received the signal
	 ...
	 */
	idol_pl_parser_table_signals[PLAYLIST_STARTED] =
		g_signal_new ("playlist-started",
			      G_TYPE_FROM_CLASS (klass),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (IdolPlParserClass, playlist_started),
			      NULL, NULL,
			      idolplparser_marshal_VOID__STRING_BOXED,
			      G_TYPE_NONE, 2, G_TYPE_STRING, IDOL_TYPE_PL_PARSER_METADATA);
	/**
	 * IdolPlParser::playlist-ended:
	 * @parser: the object which received the signal
	 * @uri: the URI of the playlist that finished parsing.
	 *
	 * The ::playlist-ended signal is emitted when a playlist is finished
	 * parsing. It is only called when #IdolPlParser::playlist-started
	 * has been called for that playlist.
	 */
	idol_pl_parser_table_signals[PLAYLIST_ENDED] =
		g_signal_new ("playlist-ended",
			      G_TYPE_FROM_CLASS (klass),
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (IdolPlParserClass, playlist_ended),
			      NULL, NULL,
			      g_cclosure_marshal_VOID__STRING,
			      G_TYPE_NONE, 1, G_TYPE_STRING);

	/* param specs */
	idol_pl_parser_pspec_pool = g_param_spec_pool_new (FALSE);
	pspec = g_param_spec_string ("url", "url",
				     "URI to be added", NULL,
				     G_PARAM_READABLE & G_PARAM_WRITABLE);
	g_param_spec_pool_insert (idol_pl_parser_pspec_pool, pspec, IDOL_TYPE_PL_PARSER);
...
static void
idol_pl_parser_base_class_finalize (IdolPlParserClass *klass)
{
	GList *list, *node;

	list = g_param_spec_pool_list_owned (idol_pl_parser_pspec_pool, G_OBJECT_CLASS_TYPE (klass));
	for (node = list; node; node = node->next) {
		GParamSpec *pspec = node->data;

		g_param_spec_pool_remove (idol_pl_parser_pspec_pool, pspec);
...
static void
idol_pl_parser_set_property (GObject *object,
			      guint prop_id,
			      const GValue *value,
			      GParamSpec *pspec)
{
	IdolPlParser *parser = IDOL_PL_PARSER (object);
...
static void
idol_pl_parser_get_property (GObject *object,
			      guint prop_id,
			      GValue *value,
			      GParamSpec *pspec)
{
...
GQuark
idol_pl_parser_error_quark (void)
{
	static GQuark quark;
	if (!quark)
		quark = g_quark_from_static_string ("idol_pl_parser_error");

	return quark;
}

static gpointer
idol_pl_parser_real_init_i18n (gpointer data)
{
...
static void
idol_pl_parser_init_i18n (void)
{
	static GOnce my_once = G_ONCE_INIT;
	g_once (&my_once, idol_pl_parser_real_init_i18n, NULL);
}

/**
 * idol_pl_parser_new:
 *
 * Creates a #IdolPlParser object.
 *
 * Return value: a new #IdolPlParser
 */
IdolPlParser *
idol_pl_parser_new (void)
{
	idol_pl_parser_init_i18n ();
	return IDOL_PL_PARSER (g_object_new (IDOL_TYPE_PL_PARSER, NULL));
}
...
/**
 * idol_pl_parser_playlist_end:
 * @parser: a #IdolPlParser
 * @playlist_uri: the playlist URI
 *
 * Emits the #IdolPlParser::playlist-ended signal on @parser for
 * the playlist @playlist_uri.
 **/
void
idol_pl_parser_playlist_end (IdolPlParser *parser, const char *playlist_uri)
{
...
static char *
my_g_file_info_get_mime_type_with_data (GFile *file, gpointer *data, IdolPlParser *parser)
{
	char *buffer;
	gsize bytes_read;
	...
	*data = buffer;
	...
	return idol_pl_parser_mime_type_from_data (*data, bytes_read);
}

/**
 * idol_pl_parser_is_debugging_enabled:
 * @parser: a #IdolPlParser
 *
 ...
 **/
gboolean
idol_pl_parser_is_debugging_enabled (IdolPlParser *parser)
{
	return parser->priv->debug;
}

/**
 * idol_pl_parser_base_uri:
 * @uri: a URI
 *
 ...
 **/
char *
idol_pl_parser_base_uri (GFile *uri)
{
	GFile *parent;
	char *ret;
 ...
 **/
gboolean
idol_pl_parser_line_is_empty (const char *line)
{
	guint i;
 ...
 * Possible error codes are as per idol_pl_parser_write_buffer().
 *
 * Return value: %TRUE on success
 **/
gboolean
idol_pl_parser_write_string (GOutputStream *stream, const char *buf, GError **error)
{
	guint len;

	len = strlen (buf);
	return idol_pl_parser_write_buffer (stream, buf, len, error);
}

/**
 * idol_pl_parser_write_buffer:
 ...
 **/
gboolean
idol_pl_parser_write_buffer (GOutputStream *stream, const char *buf, guint len, GError **error)
{
	gsize bytes_written;
 ...
 **/
int
idol_pl_parser_num_entries (IdolPlParser   *parser,
                             IdolPlPlaylist *playlist)
{
	...
	num_entries = idol_pl_playlist_size (playlist);
	...
	ignored = 0;
        ...
        while (valid) {
                ...
                if (idol_pl_parser_scheme_is_ignored (parser, file)) {
                        ignored++;
                }
...
char *
idol_pl_parser_relative (GFile *output, const char *filepath)
{
	GFile *parent, *file;
	char *retval;
...
char *
idol_pl_parser_resolve_uri (GFile *base_gfile,
 ...
 * @type: a #IdolPlParserType for the outputted playlist
 ...
 **/
gboolean
idol_pl_parser_save (IdolPlParser      *parser,
                      IdolPlPlaylist    *playlist,
                      GFile              *dest,
                      const gchar        *title,
                      ...
                      GError            **error)
{
        ...
        switch (type)
        {
	case IDOL_PL_PARSER_PLS:
		return idol_pl_parser_save_pls (parser, playlist, dest, title, error);
	case IDOL_PL_PARSER_M3U:
	case IDOL_PL_PARSER_M3U_DOS:
		return idol_pl_parser_save_m3u (parser, playlist, dest,
                                                 (type == IDOL_PL_PARSER_M3U_DOS),
                                                 error);
	case IDOL_PL_PARSER_XSPF:
		return idol_pl_parser_save_xspf (parser, playlist, dest, title, error);
	case IDOL_PL_PARSER_IRIVER_PLA:
		return idol_pl_parser_save_pla (parser, playlist, dest, title, error);
	default:
...
/**
 * idol_pl_parser_read_ini_line_int:
 ...
 **/
int
idol_pl_parser_read_ini_line_int (char **lines, const char *key)
{
	int retval = -1;
	int i;
...
/**
 * idol_pl_parser_read_ini_line_string_with_sep:
 * @lines: a NULL-terminated array of INI lines to read
 * @key: the key to match
 ...
 **/
char*
idol_pl_parser_read_ini_line_string_with_sep (char **lines, const char *key,
		const char *sep)
{
	char *retval = NULL;
	int i;
...
/**
 * idol_pl_parser_read_ini_line_string:
 * @lines: a NULL-terminated array of INI lines to read
 * @key: the key to match
 *
 ...
 **/
char*
idol_pl_parser_read_ini_line_string (char **lines, const char *key)
{
	return idol_pl_parser_read_ini_line_string_with_sep (lines, key, "=");
}

static void
idol_pl_parser_init (IdolPlParser *parser)
{
	parser->priv = G_TYPE_INSTANCE_GET_PRIVATE (parser, IDOL_TYPE_PL_PARSER, IdolPlParserPrivate);
	parser->priv->ignore_mutex = g_mutex_new ();
	parser->priv->main_thread = g_thread_self ();
}

static void
idol_pl_parser_finalize (GObject *object)
{
	IdolPlParserPrivate *priv = IDOL_PL_PARSER (object)->priv;
	...
	G_OBJECT_CLASS (idol_pl_parser_parent_class)->finalize (object);
}
...
static void
idol_pl_parser_add_uri_valist (IdolPlParser *parser,
				...
				va_list      var_args)
{
	...
	while (name) {
		...
		pspec = g_param_spec_pool_lookup (idol_pl_parser_pspec_pool,
						  name,
		...
		if (strcmp (name, IDOL_PL_PARSER_FIELD_URI) == 0) {
			if (uri == NULL)
				uri = g_value_dup_string (&value);
		} else if (strcmp (name, IDOL_PL_PARSER_FIELD_FILE) == 0) {
			GFile *file;
			...
			name = va_arg (var_args, char*);
			continue;
		} else if (strcmp (name, IDOL_PL_PARSER_FIELD_BASE_FILE) == 0) {
			GFile *file;
			char *base_uri;
			...
			g_hash_table_insert (metadata,
					     g_strdup (IDOL_PL_PARSER_FIELD_BASE),
					     base_uri);

			g_value_unset (&value);
			name = va_arg (var_args, char*);
			continue;
		} else if (strcmp (name, IDOL_PL_PARSER_FIELD_IS_PLAYLIST) == 0) {
	...
	if (g_hash_table_size (metadata) > 0 || uri != NULL) {
		...
		if (is_playlist == FALSE)
			data->signal_id = idol_pl_parser_table_signals[ENTRY_PARSED];
		else
			data->signal_id = idol_pl_parser_table_signals[PLAYLIST_STARTED];
 ...
 **/
void
idol_pl_parser_add_uri (IdolPlParser *parser,
			 const char *first_property_name,
			 ...)
{
	va_list var_args;
	va_start (var_args, first_property_name);
	idol_pl_parser_add_uri_valist (parser, first_property_name, var_args);
	va_end (var_args);
}

/**
 * idol_pl_parser_add_one_uri:
 * @parser: a #IdolPlParser
 * @uri: the entry's URI
 ...
 **/
void
idol_pl_parser_add_one_uri (IdolPlParser *parser, const char *uri, const char *title)
{
...
void
idol_pl_parser_add_one_file (IdolPlParser *parser, GFile *file, const char *title)
{
	idol_pl_parser_add_uri (parser,
				 IDOL_PL_PARSER_FIELD_FILE, file,
				 IDOL_PL_PARSER_FIELD_TITLE, title,
				 NULL);
}
...
/**
 * idol_pl_parser_scheme_is_ignored:
 * @parser: a #IdolPlParser
 * @uri: a URI
 *
 ...
 **/
gboolean
idol_pl_parser_scheme_is_ignored (IdolPlParser *parser, GFile *uri)
{
	GList *l;
...
static gboolean
idol_pl_parser_mimetype_is_ignored (IdolPlParser *parser,
				     const char *mimetype)
{
	GList *l;
 ...
 * </itemizedlist>
 *
 * URIs are automatically ignored if their scheme is ignored as per idol_pl_parser_scheme_is_ignored(),
 * and are ignored if all the other tests are inconclusive.
 *
 * Return value: %TRUE if @uri is to be ignored
 **/
gboolean
idol_pl_parser_ignore (IdolPlParser *parser, const char *uri)
{
	char *mimetype;
	GFile *file;
	guint i;
 ...
 */
static void
idol_pl_parser_cleanup_xml (char *contents)
{
	char *needle;
...
xml_node_t *
idol_pl_parser_parse_xml_relaxed (char *contents,
				   gsize size)
{
	xml_node_t* doc, *node;
	...
	idol_pl_parser_cleanup_xml (contents);
...
static gboolean
idol_pl_parser_ignore_from_mimetype (IdolPlParser *parser, const char *mimetype)
{
	guint i;
...
idol_pl_parser_get_function_for_mimetype (const char *mimetype)
{
	guint i;
...
IdolPlParserResult
idol_pl_parser_parse_internal (IdolPlParser *parser,
				GFile *file,
				GFile *base_file,
				IdolPlParseData *parse_data)
{
	...
	guint i;
	gpointer data = NULL;
	IdolPlParserResult ret = IDOL_PL_PARSER_RESULT_UNHANDLED;
	...
	if (g_file_has_uri_scheme (file, "itpc") != FALSE
		...
		return idol_pl_parser_add_itpc (parser, file, base_file, parse_data, NULL);
	}
	if (g_file_has_uri_scheme (file, "zune") != FALSE) {
		DEBUG(file, g_print ("URI '%s' is getting special cased for ZUNE parsing\n", uri));
		return idol_pl_parser_add_zune (parser, file, base_file, parse_data, NULL);
	}
	/* Try itms Podcast references, see itunes.py in PenguinTV */
	if (idol_pl_parser_is_itms_feed (file) != FALSE) {
		DEBUG(file, g_print ("URI '%s' is getting special cased for ITMS parsing\n", uri));
		return idol_pl_parser_add_itms (parser, file, NULL, parse_data, NULL);
	}
	...
	if (g_file_has_uri_scheme (file, "http")) {
		...
		if (url != NULL && idol_pl_parser_is_videosite (url, parser->priv->debug) != FALSE) {
			ret = idol_pl_parser_add_videosite (parser, file, base_file, parse_data, NULL);
	...
	if (idol_pl_parser_mimetype_is_ignored (parser, mimetype) != FALSE) {
		g_free (mimetype);
		g_free (data);
	...
	if (parse_data->recurse || parse_data->recurse_level == 0) {
		...
		for (i = 0; i < G_N_ELEMENTS(dual_types) && found == FALSE; i++) {
			if (strcmp (dual_types[i].mimetype, mimetype) == 0) {
				...
				func = idol_pl_parser_get_function_for_mimetype (mimetype);
				if ((func == NULL && mimetype != NULL) || (mimetype == NULL && dual_types[i].func == NULL)) {
					...
					ret = IDOL_PL_PARSER_RESULT_UNHANDLED;
					g_free (mimetype);
					mimetype = NULL;
					break;
				} else if (func == NULL) {
	...
	if (idol_pl_parser_ignore_from_mimetype (parser, mimetype) != FALSE) {
		g_free (mimetype);
		return IDOL_PL_PARSER_RESULT_IGNORED;
	}
	g_free (mimetype);

	if (ret != IDOL_PL_PARSER_RESULT_SUCCESS && parse_data->fallback) {
		idol_pl_parser_add_one_file (parser, file, NULL);
		return IDOL_PL_PARSER_RESULT_SUCCESS;
	}
...
{
	IdolPlParserResult parse_result;
	GError *error = NULL;
	...
	if (g_cancellable_set_error_if_cancelled (cancellable, &error) == TRUE) {
		...
		g_simple_async_result_set_op_res_gpointer (result, GUINT_TO_POINTER (IDOL_PL_PARSER_RESULT_CANCELLED), NULL);
		g_error_free (error);
		return;
	}

	/* Parse and return */
	parse_result = idol_pl_parser_parse_with_base (IDOL_PL_PARSER (object), data->uri, data->base, data->fallback);
 ...
 *
 * For more details, see idol_pl_parser_parse_with_base(), which is the synchronous version of this function.
 *
 * When the operation is finished, @callback will be called. You can then call idol_pl_parser_parse_finish()
 * to get the results of the operation.
 **/
void
idol_pl_parser_parse_with_base_async (IdolPlParser *parser, const char *uri, const char *base, gboolean fallback,
				       GCancellable *cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
	GSimpleAsyncResult *result;
	ParseAsyncData *data;

	g_return_if_fail (IDOL_IS_PL_PARSER (parser));
	...
	result = g_simple_async_result_new (G_OBJECT (parser), callback, user_data, idol_pl_parser_parse_with_base_async);
 ...
 *
 * Return value: a #IdolPlParserResult
 **/
IdolPlParserResult
idol_pl_parser_parse_with_base (IdolPlParser *parser, const char *uri,
				 const char *base, gboolean fallback)
{
	GFile *file, *base_file;
	IdolPlParserResult retval;
	IdolPlParseData data;

	g_return_val_if_fail (IDOL_IS_PL_PARSER (parser), IDOL_PL_PARSER_RESULT_UNHANDLED);
	g_return_val_if_fail (uri != NULL, IDOL_PL_PARSER_RESULT_UNHANDLED);
	...
	retval = idol_pl_parser_parse_internal (parser, file, base_file, &data);
 ...
 *
 * For more details, see idol_pl_parser_parse(), which is the synchronous version of this function.
 *
 * When the operation is finished, @callback will be called. You can then call idol_pl_parser_parse_finish()
 * to get the results of the operation.
 **/
void
idol_pl_parser_parse_async (IdolPlParser *parser, const char *uri, gboolean fallback,
			     GCancellable *cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
	idol_pl_parser_parse_with_base_async (parser, uri, NULL, fallback, cancellable, callback, user_data);
}
 ...
 *
 * Finishes an asynchronous playlist parsing operation started with idol_pl_parser_parse_async()
 * or idol_pl_parser_parse_with_base_async().
 *
 * If parsing of the playlist is cancelled part-way through, %IDOL_PL_PARSER_RESULT_CANCELLED is returned when
 * this function is called.
 *
 * Return value: a #IdolPlParserResult
 **/
IdolPlParserResult
idol_pl_parser_parse_finish (IdolPlParser *parser, GAsyncResult *async_result, GError **error)
{
	...
	g_warn_if_fail (g_simple_async_result_get_source_tag (result) == idol_pl_parser_parse_with_base_async);
 ...
 * servers. idol_pl_parser_parse_async() is recommended instead.
 *
 * Return values are as idol_pl_parser_parse_with_base().
 *
 * Return value: a #IdolPlParserResult
 **/
IdolPlParserResult
idol_pl_parser_parse (IdolPlParser *parser, const char *uri,
		       gboolean fallback)
{
	return idol_pl_parser_parse_with_base (parser, uri, NULL, fallback);
}
 ...
 **/
void
idol_pl_parser_add_ignored_scheme (IdolPlParser *parser,
		const char *scheme)
{
	char *s;
 ...
 **/
void
idol_pl_parser_add_ignored_mimetype (IdolPlParser *parser,
		const char *mimetype)
{
	g_return_if_fail (IDOL_IS_PL_PARSER (parser));
 ...
 **/
gint64
idol_pl_parser_parse_duration (const char *duration, gboolean debug)
{
 ...
 **/
guint64
idol_pl_parser_parse_date (const char *date_str, gboolean debug)
{
#ifdef HAVE_GMIME
	GTimeVal val;
...
static char *
idol_pl_parser_mime_type_from_data (gconstpointer data, int len)
{
	char *mime_type;
	gboolean uncertain;
...
/**
 * idol_pl_parser_can_parse_from_data:
 ...
 * and precedences as idol_pl_parser_ignore().
 *
 * Return value: %TRUE if @data can be parsed
 **/
gboolean
idol_pl_parser_can_parse_from_data (const char *data,
				     gsize len,
				     gboolean debug)
{
	char *mimetype;
	...
	/* Bad cast! */
	mimetype = idol_pl_parser_mime_type_from_data ((gpointer) data, (int) len);

	if (mimetype == NULL) {
		D(g_message ("idol_pl_parser_can_parse_from_data couldn't get mimetype"));
		return FALSE;
	}
...
idol_pl_parser_can_parse_from_filename (const char *filename, gboolean debug)
{
	...
	retval = idol_pl_parser_can_parse_from_data
...
/**
 * idol_pl_parser_can_parse_from_uri:
 ...
 **/
gboolean
idol_pl_parser_can_parse_from_uri (const char *uri, gboolean debug)
{
	return idol_pl_parser_is_videosite (uri, debug);
}

#ifndef IDOL_PL_PARSER_MINI
GType
idol_pl_parser_metadata_get_type (void)
{
	static volatile gsize g_define_type_id__volatile = 0;
	if (g_once_init_enter (&g_define_type_id__volatile))
	{
		GType g_define_type_id = g_boxed_type_register_static (
		    g_intern_static_string ("IdolPlParserMetadata"),
