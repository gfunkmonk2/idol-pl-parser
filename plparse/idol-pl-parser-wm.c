...
const char *
idol_pl_parser_is_asx (const char *data, gsize len)
{
	if (len == 0)
		return NULL;
...
const char *
idol_pl_parser_is_asf (const char *data, gsize len)
{
	if (len == 0)
		return NULL;
	...
	return idol_pl_parser_is_asx (data, len);
}

#ifndef IDOL_PL_PARSER_MINI

static IdolPlParserResult
idol_pl_parser_add_asf_reference_parser (IdolPlParser *parser,
					  GFile *file,
					  GFile *base_file,
					  ...
					  gpointer data)
{
	...
	ref = idol_pl_parser_read_ini_line_string (lines, "Ref1");
	if (ref == NULL) {
		g_strfreev (lines);
		return idol_pl_parser_add_asx (parser, file, base_file, parse_data, data);
	}
	...
	idol_pl_parser_add_one_uri (parser, ref, NULL);
	g_free (ref);
...
static IdolPlParserResult
idol_pl_parser_add_asf_parser (IdolPlParser *parser,
				GFile *file,
				GFile *base_file,
				IdolPlParseData *parse_data,
				gpointer data)
{
	...
	if (g_str_has_prefix (data, "ASF ") == FALSE) {
		return idol_pl_parser_add_asf_reference_parser (parser, file, base_file, parse_data, data);
	}
...
static gboolean
parse_asx_entry (IdolPlParser *parser, GFile *base_file, xml_node_t *parent, IdolPlParseData *parse_data)
{
	xml_node_t *node;
	IdolPlParserResult retval = IDOL_PL_PARSER_RESULT_SUCCESS;
	GFile *resolved;
	char *resolved_uri;
	const char *uri;
	...
	for (node = parent->child; node != NULL; node = node->next) {
		...
		if (g_ascii_strcasecmp (node->name, "param") == 0) {
			...
			retval = IDOL_PL_PARSER_RESULT_IGNORED;
			goto bail;
		}
	}
	...
	retval = idol_pl_parser_parse_internal (parser, resolved, NULL, parse_data);
	if (retval != IDOL_PL_PARSER_RESULT_SUCCESS) {
		idol_pl_parser_add_uri (parser,
					 ...
					 IDOL_PL_PARSER_FIELD_MOREINFO, moreinfo,
					 NULL);
		retval = IDOL_PL_PARSER_RESULT_SUCCESS;
	}
...
static gboolean
parse_asx_entryref (IdolPlParser *parser, GFile *base_file, xml_node_t *node, IdolPlParseData *parse_data)
{
	IdolPlParserResult retval = IDOL_PL_PARSER_RESULT_SUCCESS;
	const char *uri;
	GFile *resolved;
...
static gboolean
parse_asx_entries (IdolPlParser *parser, const char *uri, GFile *base_file, xml_node_t *parent, IdolPlParseData *parse_data)
{
	char *title = NULL;
	GFile *new_base;
	xml_node_t *node;
...
IdolPlParserResult
idol_pl_parser_add_asx (IdolPlParser *parser,
			 GFile *file,
			 GFile *base_file,
			 IdolPlParseData *parse_data,
			 gpointer data)
{
	...
	if (data != NULL && idol_pl_parser_is_uri_list (data, strlen (data)) != FALSE) {
		return idol_pl_parser_add_ram (parser, file, parse_data, data);
	}
...
IdolPlParserResult
idol_pl_parser_add_asf (IdolPlParser *parser,
			 GFile *file,
			 GFile *base_file,
			 IdolPlParseData *parse_data,
			 gpointer data)
{
	...
	if (idol_pl_parser_is_asf (data, strlen (data)) == FALSE) {
		idol_pl_parser_add_one_file (parser, file, NULL);
		return IDOL_PL_PARSER_RESULT_SUCCESS;
	}

	return idol_pl_parser_add_asf_parser (parser, file, base_file, parse_data, data);
}
