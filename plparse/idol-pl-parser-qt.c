...
const char *
idol_pl_parser_is_quicktime (const char *data, gsize len)
{
	if (len == 0)
		return FALSE;
...
idol_pl_parser_add_quicktime_rtsptext (IdolPlParser *parser,
					GFile *file,
					GFile *base_file,
					...
					gpointer data)
{
	...
	volume = idol_pl_parser_read_ini_line_string_with_sep
		(lines, "volume", "=");
	autoplay = idol_pl_parser_read_ini_line_string_with_sep
		(lines, "autoplay", "=");
	...
	idol_pl_parser_add_uri (parser,
				 IDOL_PL_PARSER_FIELD_URI, rtspuri,
				 IDOL_PL_PARSER_FIELD_VOLUME, volume,
...
IdolPlParserResult
idol_pl_parser_add_quicktime (IdolPlParser *parser,
			       GFile *file,
			       GFile *base_file,
			       IdolPlParseData *parse_data,
			       gpointer data)
{
	if (data == NULL || idol_pl_parser_is_quicktime (data, strlen (data)) == NULL)
		return IDOL_PL_PARSER_RESULT_UNHANDLED;

	return idol_pl_parser_add_quicktime_metalink (parser, file, base_file, parse_data, data);
}
