...
static struct {
	...
	{ IDOL_PL_PARSER_FIELD_AUTHOR, "creator" },
	{ IDOL_PL_PARSER_FIELD_IMAGE_URI, "image" },
	{ IDOL_PL_PARSER_FIELD_ALBUM, "album" },
	{ IDOL_PL_PARSER_FIELD_DURATION_MS, "duration" },
	{ IDOL_PL_PARSER_FIELD_GENRE, NULL },
};

gboolean
idol_pl_parser_save_xspf (IdolPlParser    *parser,
                           ...
                           GError          **error)
{
	...
	if (success == FALSE)
		...
		for (i = 0; i < G_N_ELEMENTS (fields); i++) {
			...
			if (!escaped)
				continue;
			if (g_str_equal (fields[i].field, IDOL_PL_PARSER_FIELD_GENRE) == FALSE) {
		...
		success = idol_pl_parser_write_string (G_OUTPUT_STREAM (stream), "  </track>\n", error);
		if (success == FALSE)
			return FALSE;
...
static gboolean
parse_xspf_track (IdolPlParser *parser, GFile *base_file, xmlDocPtr doc,
		xmlNodePtr parent)
{
	xmlNodePtr node;
	...
	if (uri == NULL) {
		retval = IDOL_PL_PARSER_RESULT_ERROR;
		goto bail;
	}

	resolved_uri = idol_pl_parser_resolve_uri (base_file, (char *) uri);
	...
	idol_pl_parser_add_uri (parser,
				 ...
				 IDOL_PL_PARSER_FIELD_TITLE, title,
				 IDOL_PL_PARSER_FIELD_DURATION_MS, duration,
				 IDOL_PL_PARSER_FIELD_IMAGE_URI, image_uri,
				 IDOL_PL_PARSER_FIELD_AUTHOR, artist,
				 IDOL_PL_PARSER_FIELD_ALBUM, album,
				 IDOL_PL_PARSER_FIELD_MOREINFO, moreinfo,
				 IDOL_PL_PARSER_FIELD_DOWNLOAD_URI, download_uri,
...
static gboolean
parse_xspf_trackList (IdolPlParser *parser, GFile *base_file, xmlDocPtr doc,
		xmlNodePtr parent)
{
	xmlNodePtr node;
	IdolPlParserResult retval = IDOL_PL_PARSER_RESULT_ERROR;
...
static gboolean
parse_xspf_entries (IdolPlParser *parser, GFile *base_file, xmlDocPtr doc,
		xmlNodePtr parent)
{
	xmlNodePtr node;
	IdolPlParserResult retval = IDOL_PL_PARSER_RESULT_ERROR;
...
IdolPlParserResult
idol_pl_parser_add_xspf (IdolPlParser *parser,
			  GFile *file,
			  GFile *base_file,
			  ...
			  gpointer data)
{
	...
	doc = idol_pl_parser_parse_xml_file (file);
