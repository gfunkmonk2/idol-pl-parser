...
static char *
idol_pl_parser_uri_to_dos (const char *uri, GFile *output)
{
	char *retval, *i;

	/* Get a relative URI if there is one */
	retval = idol_pl_parser_relative (output, uri);
...
static void
idol_pl_parser_parse_ram_uri (IdolPlParser *parser, const char *uri)
{
	char *mark, **params;
	GString *str;
	guint i, num_params;
	...
	if (g_str_has_prefix (uri, "rtsp://") == FALSE
		...
		idol_pl_parser_add_one_uri (parser, uri, NULL);
		return;
	}
	...
	if (mark[1] == '\0') {
		...
		idol_pl_parser_add_one_uri (parser, new_uri, NULL);
		g_free (new_uri);
		return;
	}
	...
	idol_pl_parser_add_uri (parser,
				 IDOL_PL_PARSER_FIELD_URI, str->str,
				 IDOL_PL_PARSER_FIELD_TITLE, title,
				 IDOL_PL_PARSER_FIELD_AUTHOR, author,
				 IDOL_PL_PARSER_FIELD_COPYRIGHT, copyright,
				 IDOL_PL_PARSER_FIELD_ABSTRACT, abstract,
				 IDOL_PL_PARSER_FIELD_SCREENSIZE, screensize,
				 IDOL_PL_PARSER_FIELD_UI_MODE, mode,
				 IDOL_PL_PARSER_FIELD_STARTTIME, start,
				 IDOL_PL_PARSER_FIELD_ENDTIME, end,
				 NULL);
...
IdolPlParserResult
idol_pl_parser_add_ram (IdolPlParser *parser, GFile *file, IdolPlParseData *parse_data, gpointer data)
{
	gboolean retval = IDOL_PL_PARSER_RESULT_UNHANDLED;
	char *contents, **lines;
	gsize size;
	guint i;
	...
	for (i = 0; lines[i] != NULL; i++) {
		/* Empty line */
		if (idol_pl_parser_line_is_empty (lines[i]) != FALSE)
			continue;
		...
		if (strstr(lines[i], "://") != NULL
			...
			if (idol_pl_parser_parse_internal (parser, line_file, NULL, parse_data) != IDOL_PL_PARSER_RESULT_SUCCESS)
				idol_pl_parser_parse_ram_uri (parser, lines[i]);
			...
			/* Try with a base */
			base = idol_pl_parser_base_uri (uri);

			if (idol_pl_parser_parse_internal (parser, lines[i], base) != IDOL_PL_PARSER_RESULT_SUCCESS)
			{
				char *fullpath;
				fullpath = g_strdup_printf ("%s/%s", base, lines[i]);
				idol_pl_parser_parse_ram_uri (parser, fullpath);
				g_free (fullpath);
			}
			g_free (base);
#endif
...
static const char *
idol_pl_parser_get_extinfo_title (const char *extinfo)
{
	const char *res, *sep;
...
IdolPlParserResult
idol_pl_parser_add_ra (IdolPlParser *parser,
			GFile *file,
			GFile *base_file,
			IdolPlParseData *parse_data,
			gpointer data)
{
	if (data == NULL || idol_pl_parser_is_uri_list (data, strlen (data)) == NULL) {
		idol_pl_parser_add_one_file (parser, file, NULL);
		return IDOL_PL_PARSER_RESULT_SUCCESS;
	}

	return idol_pl_parser_add_ram (parser, file, parse_data, NULL);
}
...
const char *
idol_pl_parser_is_uri_list (const char *data, gsize len)
{
	guint i = 0;
