...
#ifndef IDOL_PL_PARSER_MINI
gboolean
idol_pl_parser_save_pls (IdolPlParser    *parser,
                          ...
                          GError          **error)
{
	...
	num_entries = idol_pl_parser_num_entries (parser, playlist);
	...
	if (success == FALSE)
        ...
        while (valid) {
                ...
                relative = idol_pl_parser_relative (output, uri);
...
IdolPlParserResult
idol_pl_parser_add_pls_with_contents (IdolPlParser *parser,
				       GFile *file,
				       GFile *_base_file,
				       const char *contents,
				       IdolPlParseData *parse_data)
{
	...
	/* Ignore empty lines */
	while (lines[i] != NULL && idol_pl_parser_line_is_empty (lines[i]) != FALSE)
		i++;
	...
	playlist_title = idol_pl_parser_read_ini_line_string (lines,
							       "X-MATE-Title");
	...
	if (playlist_title != NULL) {
		idol_pl_parser_add_uri (parser,
					 ...
					 IDOL_PL_PARSER_FIELD_TITLE, playlist_title,
					 NULL);
	}
	...
	for (i = 0; lines[i] != NULL; i++) {
		...
		if (idol_pl_parser_line_is_empty (lines[i]))
			continue;
	...
	for (i = 1; found_entries < num_entries; i++) {
		...
		if (length != NULL)
			length_num = idol_pl_parser_parse_duration (length, idol_pl_parser_is_debugging_enabled (parser));

		if (strstr (file_str, "://") != NULL || file_str[0] == G_DIR_SEPARATOR) {
			GFile *target;

			target = g_file_new_for_commandline_arg (file_str);
			if (length_num < 0 || idol_pl_parser_parse_internal (parser, target, NULL, parse_data) != IDOL_PL_PARSER_RESULT_SUCCESS) {
				idol_pl_parser_add_uri (parser,
							 IDOL_PL_PARSER_FIELD_URI, file_str,
							 IDOL_PL_PARSER_FIELD_TITLE, title,
							 IDOL_PL_PARSER_FIELD_GENRE, genre,
							 IDOL_PL_PARSER_FIELD_DURATION, length,
							 IDOL_PL_PARSER_FIELD_BASE_FILE, base_file, NULL);
			}
			...
			if (length_num < 0 || idol_pl_parser_parse_internal (parser, target, base_file, parse_data) != IDOL_PL_PARSER_RESULT_SUCCESS) {
				idol_pl_parser_add_uri (parser,
							 IDOL_PL_PARSER_FIELD_FILE, target,
							 IDOL_PL_PARSER_FIELD_TITLE, title,
							 IDOL_PL_PARSER_FIELD_GENRE, genre,
							 IDOL_PL_PARSER_FIELD_DURATION, length,
							 IDOL_PL_PARSER_FIELD_BASE_FILE, base_file, NULL);
	...
	if (playlist_title != NULL)
		idol_pl_parser_playlist_end (parser, playlist_title);
...
IdolPlParserResult
idol_pl_parser_add_pls (IdolPlParser *parser,
			 GFile *file,
			 GFile *base_file,
			 IdolPlParseData *parse_data,
			 gpointer data)
